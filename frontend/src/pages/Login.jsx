...
// Modus clients shown on the login page. Logos are served as uniform monochrome
// silhouettes pre-generated by scripts/normalize-logos.mjs (trimmed, recolored to
// one ink, dark backgrounds stripped) so a heterogeneous set reads as one strip.
//   file  — logo filename when it differs from `<slug>.png`
//   scale — per-logo size multiplier so every logo occupies a similar *footprint*,
//           not the same height. Derived by equal-area normalization from each
//           logo's aspect ratio (wide wordmarks shorter, compact marks taller),
//           so none dominates. See scripts/normalize-logos.mjs. Tune by eye.
const LOGO_DIR = "/clients/normalized";
const BASE_H = 24; // px — reference render height; `scale` adjusts per logo
const CLIENTS = [
  { name: "Arcaffe", slug: "arcaffe", scale: 0.75 },
  { name: "Agadir", slug: "agadir", scale: 1.1 },
  { name: "Armani", slug: "armani", scale: 0.55 },
  { name: "Boss", slug: "boss", scale: 0.9 },
  { name: "Boutique Central", slug: "boutique-central", file: "boutiquecentral.png", scale: 1.15 },
  { name: "Japanika", slug: "japanika", scale: 0.7 },
  { name: "Lululemon", slug: "lululemon", scale: 1.6 },
  { name: "Michael Kors", slug: "michael-kors", scale: 0.5 },
  { name: "Polo Ralph Lauren", slug: "polo-ralph-lauren", scale: 0.5 },
  { name: "Puma", slug: "puma", scale: 0.9 },
  { name: "Pasta Basta", slug: "pasta-basta", file: "pastabasta.png", scale: 1.5 },
  { name: "Sweetime", slug: "sweetime" },
  { name: "Tommy Hilfiger", slug: "tommy-hilfiger", scale: 0.55 },
];
...
// Logos are height-normalized (uniform render height, intrinsic width) rather
// than boxed, since height is what the eye reads as size; `scale` corrects the
// remaining optical imbalance. The assets are already one ink, so the wall just
// dims at rest and resolves to full opacity on hover.
function ClientLogo({ name, slug, file, scale = 1 }) {
  ...
  return (
    <img
      src={`${LOGO_DIR}/${file || `${slug}.png`}`}
      alt={name}
      onError={() => setErrored(true)}
      style={{ height: `${BASE_H * scale}px` }}
      className="w-auto max-w-[240px] object-contain opacity-60 hover:opacity-100 transition-opacity duration-200"
    />
  );
}
      ...
      <div className="flex flex-1 min-h-screen w-full bg-white rounded-none overflow-hidden border-none shadow-none flex-col md:flex-row">
        ...
        <div className="flex-1 bg-white flex flex-col items-center justify-center relative overflow-hidden p-6 md:p-12 h-[300px] md:h-auto order-1 md:order-2">
          ...
          <div className="flex flex-col items-center justify-center gap-6 z-10 relative">
            ...
            {/* Partner Container — Modus clients (height-normalized strip) */}
            <div className="partner-container w-full max-w-2xl mt-4">
              <div className="flex flex-wrap items-center justify-center gap-x-8 gap-y-5">
                {CLIENTS.map((c) => (
                  <ClientLogo key={c.slug} {...c} />
                ))}
              </div>
            </div>
          </div>
